...
public class Joystick extends View implements Runnable {
	...
	public int xPosition = 0; // Touch x position
	public int yPosition = 0; // Touch y position
	public double centerX = 0; // Center view x position
	public double centerY = 0; // Center view y position
	...
	public Joystick(Context context, MotionEvent me) {
		super(context);
	}
	...
	private int getAngle() {
		if (xPosition > centerX) {
			if (yPosition < centerY) {
				...
				return lastAngle = (int) (Math.atan((yPosition - centerY)
						/ (xPosition - centerX)) * RAD) * (-1);
			} else {
				return lastAngle = 0;
			}
		} else if (xPosition < centerX) {
			if (yPosition < centerY) {
				return lastAngle = (int) (180-Math.atan((yPosition - centerY)
						/ (xPosition - centerX))
						* RAD);
			} else if (yPosition > centerY) {
				return lastAngle = (int) (180+Math.atan((yPosition - centerY)
...
//	protected void Layout(boolean changed, int left, int top, int right, int bottom) {
//	    if(changed) {
//	      //  int amount = getWidth();
//	       // right -= amount;
//	        //left -= amount;
